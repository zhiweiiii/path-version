...
def scan_software_versions():
    ...
    software_versions.update(python_versions)
    
    print("扫描Node.js版本")
    node_versions = scan_node_versions()
    print(f"Node.js版本: {node_versions}")
    software_versions.update(node_versions)
    
    print("扫描Java版本")
    java_versions = scan_java_versions()
    print(f"Java版本: {java_versions}")
    software_versions.update(java_versions)
    
    print("扫描Git版本")
    git_versions = scan_git_versions()
    print(f"Git版本: {git_versions}")
    software_versions.update(git_versions)
    
    print(f"扫描到的所有软件版本: {software_versions}")
...
def run_command(cmd, cwd=None):
    ...
    print(f"命令执行结果: stdout={stdout}, stderr={stderr}, returncode={exit_code.value}")
...
def scan_python_versions():
    ...
    try:
        ...
        print(f"找到的Python可执行文件路径: {python_paths}")
        
        for path in python_paths:
            print(f"检查Python可执行文件: {path}")
            if os.path.exists(path):
                print(f"Python可执行文件存在: {path}")
                # 获取Python版本
                try:
                    print(f"运行命令获取Python版本: {path} --version")
                    stdout, stderr, returncode = run_command([path, "--version"])
                    print(f"命令执行结果: stdout={stdout}, stderr={stderr}, returncode={returncode}")
                    if returncode == 0:
                        version_match = re.search(r"Python (\d+\.\d+\.\d+)", stderr)
                        print(f"版本匹配结果: {version_match}")
                        if version_match:
