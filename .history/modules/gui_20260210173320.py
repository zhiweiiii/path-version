...
def init_data(enabled_versions):
    ...
    print(f"扫描到的软件版本: {software_versions}")
    ...
    for software, versions in software_versions.items():
        print(f"检查软件: {software}")
        for version, path in versions.items():
            print(f"检查版本: {version}, 路径: {path}")
            # 获取可执行文件所在目录
            ...
            for i, dir_path in enumerate(path_dirs):
                if dir_path.strip() == exe_dir.strip():
                    ...
                    if software not in enabled_versions:
                        ...
                        print(f"设置软件 {software} 的启用版本为: {version}")
                    break
    
    print(f"最终的启用版本: {enabled_versions}")
...
def update_table(filter_item):
    """更新表格数据"""
    print(f"开始更新表格，过滤项: {filter_item}")
    
    # 初始化启用状态
    init_enabled_status()
    
    # 清空表格
    print(f"清空表格，当前子项数量: {len(app_data.tree.get_children())}")
    ...
    print(f"清空映射后，version_paths: {app_data.version_paths}, item_key_map: {app_data.item_key_map}")
    
    # 填充数据
    print(f"开始填充数据，software_versions: {app_data.software_versions}")
    for software, versions in app_data.software_versions.items():
        # 过滤
        if filter_item != 'All' and software != filter_item:
            print(f"跳过软件: {software}，因为过滤项为: {filter_item}")
            continue
        
        print(f"处理软件: {software}")
        for version, path in versions.items():
            print(f"处理版本: {version}, 路径: {path}")
            # 获取路径的目录部分
            ...
            print(f"软件 {software} 版本 {version} 的状态: {status}")
            ...
            print(f"添加到表格: item_id={item_id}, values=({package_name}, {version}, {status})")
    
    print(f"表格更新完成，最终子项数量: {len(app_data.tree.get_children())}")
...
def create_gui():
    ...
    print("开始初始化参数")
    init_parameters()
    print(f"初始化参数完成，app_data: {app_data.__dict__}")
