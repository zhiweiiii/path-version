...
def create_gui(software_versions):
    ...
    # 导航栏标题
    nav_header = ttk.Label(
        sidebar_frame, 
        text="版本切换工具", 
        style='Header.TLabel'
    )
    nav_header.pack(anchor=tk.W, padx=20, pady=20)
    ...
    # 添加新样式
    style.configure('ContentHeader.TFrame', background='#ffffff', relief='raised', borderwidth=1)
    style.configure('ContentHeader.TLabel', font=('微软雅黑', 14, 'bold'), foreground='#333333', background='#ffffff')
    style.configure('TableContainer.TFrame', background='#ffffff', relief='raised', borderwidth=1)
    
    # 内容区域标题
    content_header = ttk.Frame(content_frame, style='ContentHeader.TFrame')
    content_header.pack(fill=tk.X, padx=0, pady=0)
    ...
    header_label.pack(anchor=tk.W, padx=30, pady=20)
    ...
    # 表格列
    columns = ('package', 'version', 'path', 'status', 'action')
    ...
    # 配置列宽
    tree.column('package', width=120, anchor=tk.W)
    tree.column('version', width=100, anchor=tk.CENTER)
    tree.column('path', width=350, anchor=tk.W)
        ...
        for software, versions in software_versions.items():
            ...
            for version, path in versions.items():
                ...
                activate_button = ttk.Button(
                    ...
                    width=8,
                    command=lambda s=software, v=version, p=path: activate_version(s, v, p)
                )
                activate_button.pack(anchor=tk.CENTER, padx=5, pady=5)
                
                # 将按钮嵌入表格
                tree.set(item_id, 'action', '')
                tree.window_create(item_id, window=action_frame, column='action')
    ...
    # 添加刷新框架样式
    style.configure('RefreshFrame.TFrame', background='#ffffff')
    
    # 刷新按钮
    refresh_frame = ttk.Frame(content_header, style='RefreshFrame.TFrame')
    refresh_frame.pack(side=tk.RIGHT, padx=20, pady=20)
